    ...
    def get_context_data(users):
        ...
        for user in users:
            employee_details.append({
                ...
                "year_of_experience": 0 if (date.fromtimestamp(time.time()).year -
                                            user.date_joined.year) < 1
                                    else str(date.fromtimestamp(time.time()).year -
                                             user.date_joined.year) + "." +
                                    str(abs(date.fromtimestamp(time.time()).month -
